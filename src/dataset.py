...
class GeneTrajDataset(Dataset):
    def __init__(
        ...
        final_cell_types: np.ndarray = None,
    ) -> None:
        super().__init__()
        ...
        if balance and final_cell_types is not None and starting_cell_type is not None:
            all_cell_types = np.unique(final_cell_types)
            selected_final_states = final_cell_types[mask]
            cell_type_ratio = dict()
            for cell_type in all_cell_types:
                cell_type_ratio[cell_type] = round(
                    np.sum(selected_final_states == cell_type)
                    / len(selected_final_states),
                    3,
                )

            # print(cell_type_ratio)
            sample_mask = final_cell_types == starting_cell_type
            sample_mask = sample_mask & mask
            num_target_samples = int(np.sum(sample_mask) * sample_ratio)

            target_indices = np.where(sample_mask)[0]
            np.random.shuffle(target_indices)
            unselected_target_indices = target_indices[num_target_samples:]

            unselected_mask = np.zeros_like(sample_mask)
            unselected_mask[unselected_target_indices] = True
            mask = mask & (~unselected_mask)
            selected_final_states = final_cell_types[mask]
            cell_type_ratio = dict()
            for cell_type in all_cell_types:
                cell_type_ratio[cell_type] = round(
            ...
            # print("after balance", cell_type_ratio)
