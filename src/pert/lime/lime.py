...
class LIMEPertModule(PertModule):
    ...
    def get_instance_importance(
        ...
        list_of_pred.append(forward_fn(x, y))
        ...
        for masked_data, masked_samples in mask_fn.mask_sample(x, mask_value):
            ...
            expanded_y = None
            if y is not None:
                expanded_size = [to_add_size] + [-1] * (y.dim() - 1)
                expanded_y = y.expand(*expanded_size)

            list_of_pred.append(forward_fn(masked_samples[:to_add_size], expanded_y))
    ...
    def get_global_importance(
        ...
        ys = torch.split(y, 1, dim=0) if y is not None else [None] * len(xs)
        scores = []
        for inst_x, inst_y in tqdm(zip(xs, ys)):
            scores.append(
                self.get_instance_importance(
                    inst_x, inst_y, mask_fn, mask_value, forward_fn, label
                )
            )
    ...
    def update(self, x: torch.Tensor, y: torch.Tensor = None, **kwargs) -> None:
        ...
        :param y: target value, defaults to None
        :param forward_fn: model forward function f(x, y) -> loss
        :param label: label for lime explanation
        """
