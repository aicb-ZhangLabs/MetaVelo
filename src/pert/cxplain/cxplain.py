...
class CXPlainPertModule(PertModule):
    ...
    def get_importance_val(self, **kwargs) -> torch.Tensor:
        return self.cxplain_values

    def update(self, x: torch.Tensor, y: torch.Tensor = None, **kwargs) -> None:
        """update function for cxplain perturbation module

        :param x: input tensor
        :param y: target tensor, defaults to None
        :param x_eval: input tensor for evaluation
        :param y_eva: target tensor, defaults to None
        :param forward_fn: model forward function
        :param head_loss_fn: per sample head loss function f(x, y) -> loss
        :param device: device for pytorch model evaluation
        :param mask_value: mask value for perturbation (different input data shapes
                           require different mask values)
        """
        ...
        train_ds = create_dataset(
            ...
            head_loss_fn=kwargs["head_loss_fn"],
            mask_fn=mask_fn,
            mask_value=kwargs["mask_value"],
        )
        eval_ds = create_dataset(
            kwargs["x_eval"],
            kwargs.get("y_eval", None),
            forward_fn=kwargs["forward_fn"],
            head_loss_fn=kwargs["head_loss_fn"],
            mask_fn=mask_fn,
            mask_value=kwargs["mask_value"],
