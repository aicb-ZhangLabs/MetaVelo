...
@dataclass
class ApproximationPertModuleConfig:
...
class ApproximationPertModule(PertModule):
    def __init__(
        self,
        pert_num: int,
        pert_space: int,
    ...
    def update(self, x: torch.Tensor, y: torch.Tensor = None, **kwargs) -> None:
        ...
        if self.smooth_input:
            ...
            loss.backward()
            grad = x.grad.reshape((batch_sz, self.smooth_number, -1)).mean(dim=1)
        else:
            x.requires_grad_()
            loss = forward_fn(x)
            loss.backward()
        ...
        # compute approximated effects
        with torch.no_grad():
            grad = grad if not self.use_abs_grad else grad.abs()
            pert_val = self.trigger_model.get_buffer("pert_val")
            total_effects = torch.sum(grad * (pert_val.unsqueeze(0) - x), dim=0)

        self.saliency_values.add_(total_effects)
