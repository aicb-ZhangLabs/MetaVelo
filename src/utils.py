...
def visualize_pert_categorical_distribution(
    category_weights: np.ndarray, khot_weights: np.ndarray, pert_khot_vec: np.ndarray
):
    """Visualize the perturbation categorical distribution

    :param category_weights: the weights of each category
    :param pert_khot_vec: the perturbation vector
    """
    fig, axs = plt.subplots(2, 1, sharex=True)
    mask = pert_khot_vec == 1

    x = np.arange(len(category_weights))
    axs[0].bar(x, category_weights)
    axs[0].plot(x, category_weights)
    axs[0].set_title("Category weights")
    # axs[0].set_xlabel("Category")
    axs[0].set_ylabel("Weight")
    axs[0].set_yscale("log")
    axs[0].scatter(x[mask], category_weights[mask], color="red", label="Selected")
    axs[0].legend()

    # plot selected categories
    axs[1].bar(x, khot_weights)
    axs[1].plot(x, khot_weights)
    axs[1].set_title("Subset weights")
    axs[1].set_xlabel("Category")
    axs[1].set_ylabel("Weight")
    axs[1].scatter(x[mask], khot_weights[mask], color="red", label="Selected")
    axs[1].legend()
    return fig, axs


def visualize_trajs(
    raw_gene_array: np.ndarray,
    raw_gene_color: np.ndarray,
    trajs: np.ndarray,
    pca_dim: int = 30,
    min_dist: float = 0.1,
    umap_neigh_num: int = 30,
):
    ...
    umap_reducer = umap.UMAP(
        n_neighbors=umap_neigh_num, n_jobs=1, random_state=42, min_dist=min_dist
    )
